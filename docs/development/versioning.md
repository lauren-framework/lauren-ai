...
`lauren-ai` uses [`setuptools-scm`](https://setuptools-scm.readthedocs.io/)
...
so that `importlib.metadata.version("lauren_ai")` always returns the
...
`lauren_ai/__init__.py` exposes `__version__`:

```python
import lauren_ai
print(lauren_ai.__version__)   # e.g. "1.2.3"
```
...
(`pip install -e .`, `pip install lauren-ai`) the metadata is present.
