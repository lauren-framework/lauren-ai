...
class TestDocumentReranker:
    ...
    def test_rerank_most_relevant_first(self):
        reranker = DocumentReranker()
        results = _make_results()
        reranked = reranker.rerank("python programming language", results)
    ...
    def test_rerank_with_top_k_none_returns_all(self):
        reranker = DocumentReranker()
        results = _make_results()
        reranked = reranker.rerank("programming", results, top_k=None)
    ...
    def test_custom_score_fn_is_used(self):
        ...
        def custom_score(query: str, doc: str) -> float:
            ...
            return len(doc)  # longer docs rank higher
    ...
    def test_custom_score_fn_affects_ranking(self):
        # Score function that ranks shorter docs first
        ...
        reranked = reranker.rerank("anything", results)
...
class TestSimpleOverlapScore:
    ...
    def test_partial_overlap_returns_fraction(self):
        ...
        # "python" overlaps (1 of 2 query words)
        assert 0.0 < score < 1.0

    def test_case_insensitive_matching(self):
        reranker = DocumentReranker()
        score_lower = reranker._simple_overlap("python", "Python is great")
        score_upper = reranker._simple_overlap("PYTHON", "Python is great")
